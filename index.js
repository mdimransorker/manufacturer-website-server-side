...
const uri = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PAS}@cluster0.mphb1.mongodb.net/?retryWrites=true&w=majority`;
...
async function run() {
  try {
    ...
    const productCollection = client.db("carpentry").collection("products");
    const orderCollection = client.db("carpentry").collection("orders");
    const paymentCollection = client.db("carpentry").collection("payments");
    const reviewCollection = client.db("carpentry").collection("reviews");
    const userCollection = client.db("carpentry").collection("users");
    const messageCollection = client.db("carpentry").collection("messages");
...
app.get("/", (req, res) => {
  res.send("Carpentry is running");
});

app.listen(port, () => {
  console.log(`Carpentry listening on port ${port}`);
});
